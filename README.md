...
This project demonstrates my MYSQL problem-solving skills through the analysis of data for Zomato, a popular food delivery company in India. The project involves setting up the database, importing data, handling null values, and solving a variety of business problems using complex SQL queries.
...
```
## EDA
-> open workbench > open connection > use database > schemas > Database(Reverse engineer > select database > select tables (then Next, Execute, Next, Finish) > Tables arrange. SAVE 
...
## Data Import (in EDA seq.)
'''sql
SCHEMAS > zomato_db > TABLES > customers (right click) > Table data import wizard > browes & import.
SELECT * FROM customers;
SCHEMAS > zomato_db > TABLES > restaurants (right click) > Table data import wizard > browes & import.
SELECT * FROM restaurants;
SCHEMAS > zomato_db > TABLES > orders (right click) > Table data import wizard > browes & import.
SELECT * FROM orders;
SCHEMAS > zomato_db > TABLES > riders (right click) > Table data import wizard > browes & import.
SELECT * FROM riders;
SCHEMAS > zomato_db > TABLES > deliveries (right click) > Table data import wizard > browes & import.
SELECT * FROM deliveries;
...
Sol: --join cm and order
SELECT *
FROM orders as o
JOIN customers as c
ON c.customer_id = o.customer_id;

-- filter for last 1 year & -- group by cm id, dishes, cnt
SELECT 
	c.customer_id,
	c.customer_name,
...
```sql
-- Approach 1 (i.g 23:55pm/2 = 11.5, floor= 11, 11*2=22, means 10pm start_time, +2 means end_time)
SELECT 
...
This project highlights my ability to handle complex MYSQL queries and provides solutions to real-world business problems in the context of a food delivery service like Zomato. The approach taken here demonstrates a structured problem-solving methodology, data manipulation skills, and the ability to derive actionable insights from data.
